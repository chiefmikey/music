live_loop :melody do
  with_fx :hpf, mix: 1, cutoff: 100, amp: 10 do
    sample :ambi_piano, finish: 0.74
    sleep 2
...
live_loop :hat do
  with_fx :slicer, mix: 1 do
    with_fx :panslicer, wave: 2, pan_max: 0.5, pulse_width: 0.5, seed: 1 do
      sample :drum_cymbal_closed, amp: 0.3, rate: 1.01, finish: 0.75, env_curve: 2
      sleep 0.25
    end
  end
end
...
live_loop :violin do
  with_fx :lpf, cutoff: 10 do
    with_fx :wobble, wave: 0, mix: 1 do
      sample :bass_thick_c, rate: 0.50, finish: 0.40, amp: 3, env_curve: 2
      sleep 1
    end
  end
end
